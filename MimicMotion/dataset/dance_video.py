...
class HumanDanceVideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pixel_values_hands = self.augmentation(
            vid_pil_image_list, self.pixel_transform, state
        )
...
class MimicMotionDataset(Dataset):
    def __init__(
        self,
        sample_rate,
        n_sample_frames,
        ...
        if self.hand:
            self.hand_transform = transforms.Compose(
                [
                    transforms.RandomResizedCrop(
                        (height, width),
                        scale=self.img_scale,
            ...
            self.hand_mask_transform = transforms.Compose(
                [
                    transforms.RandomResizedCrop(
                        (height, width),
                        scale=self.img_scale,
    ...
    def __getitem__(self, index):
        ...
        if self.hand:
            hand_path = video_meta["hand_path"]
            hand_mask_path = video_meta["hand_mask_path"]
            
            hand_reader = VideoReader(hand_path)
            hand_pil_image_list = []
            hand_mask_reader = VideoReader(hand_mask_path)
            hand_mask_pil_image_list = []
        ...
        for index in batch_index:
            ...
            pose_pil_image_list.append(Image.fromarray(img.asnumpy()))
            if self.hand:
                img = hand_reader[index]
                hand_pil_image_list.append(Image.fromarray(img.asnumpy()))
                img = hand_mask_reader[index]
                hand_mask_pil_image_list.append(Image.fromarray(img.asnumpy()))
        ...
        if self.hand:
            pixel_values_hand = self.augmentation(
                hand_pil_image_list, self.hand_transform, state
            )
            
            pixel_values_hand_mask = self.augmentation(
                hand_mask_pil_image_list, self.hand_mask_transform, state
            )
        ...
        if self.hand:
            sample = dict(
                ...
                pixel_values_vid=pixel_values_vid,
                pixel_values_pose=pixel_values_pose,
                pixel_values_hand=pixel_values_hand,
                pixel_values_hand_mask=pixel_values_hand_mask,
                pixel_values_ref_img=pixel_values_ref_img,
                clip_ref_img=clip_ref_img,
            )
        else:

...
from ..modules.encoder import FrozenCLIPEmbedder
...
class MimicMotionPipeline2(DiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        self.cond_stage_model = FrozenCLIPEmbedder().to(device="cuda")
    ...
    def _encode_control_image(
        ...
        # Get latents_control
        control_latents = self.control_net(control_image)

        if do_classifier_free_guidance:
            negative_control_latents = torch.zeros_like(control_latents)

            # For classifier free guidance, we need to do two forward passes.
            # Here we concatenate the unconditional and text embeddings into a single batch
            # to avoid doing two forward passes
            control_latents = torch.cat([negative_control_latents, control_latents])
    ...
    def __call__(
        ...
        hand_images: Union[torch.FloatTensor], 
        hand_masks: Union[torch.FloatTensor],
        height: int = 576,
        width: int = 1024,
        ...
        print(f"Shape of Repeat image_latents: {image_latents.shape}")
        ...
        print(f"Shape of prepared latents before tiling: {latents.shape}")
        latents = latents.repeat(1, num_frames // tile_size + 1, 1, 1, 1)[:, :num_frames]
        print(f"Shape of prepared latents after tiling: {latents.shape}")
        ...
        print(f"Shape of rearranged pose_latents: {pose_latents.shape}")
        
        # 9. hand loop
        print(hand_images.shape, hand_masks.shape)
        pixel_values_hand = hand_images.transpose(
                    1, 2
        ).to(device=device)  # (bs, c, f, H, W)
        pixel_values_hand_mask =hand_masks.transpose(
                    1, 2
        ).to(device=device)
        ...
        n_prompt = 'fake 3D rendered image, longbody, lowres, bad anatomy, bad hands, missing fingers, extra\
                digit, fewer digits, cropped, worst quality, low quality, blue'
        c_crossattn = self.cond_stage_model.encode([n_prompt] * num_samples)
        ...
        cond_txt = torch.cat(cond['c_crossattn'], 1).to(device=device)
        c_concat = torch.cat(cond['c_concat'], 1).to(device=device)
        hint = torch.cat(cond['c_control'], 1).to(device=device)
        hint_latents = self._encode_control_image(
            hint, do_classifier_free_guidance=self.do_classifier_free_guidance,
        )
        hint_latents = einops.rearrange(hint_latents, '(b f) c h w -> b f c h w', f=num_frames)
        print(f"hint_latents: {hint.shape, hint_latents.shape}")
        ...
        with self.progress_bar(total=len(timesteps) * len(indices)) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for idx in indices:
                    ...
                    print(f"Shape of encoder_hidden_states (image_embeddings): {image_embeddings.shape}")
                    ...
                    _noise_pred = self.unet(
                        ...
                        return_dict=False,
                        control_latents = hint_latents[:, idx].flatten(0, 1)*0.3,
                    )[0]
        ...
        if not output_type == "latent":
            ...
            frames = latents
            return latents, noise_pred, weight
...
#             feature_extractor=feature_extractor,
#             pose_net=pose_net,
#             control_net=control_net.to(dtype=torch.float32),
#         )
#         self.vae_scale_factor = 2 ** (len(self.vae.config.block_out_channels) - 1)
#         self.image_processor = VaeImageProcessor(vae_scale_factor=self.vae_scale_factor)
#         self.cond_stage_model = FrozenCLIPEmbedder().to(device="cuda")
...
#                 current_latent = latents[:, i, :, :, :].squeeze(2)
#                 current_hint = hint[:, i, :, :, :].squeeze(2)
#                 # print(f"current_latent shape x_noisy: {current_latent.shape, current_latent.dtype}")
#                 # print(f"current_hint shape hint: {current_hint.shape, current_hint.dtype}")
#                 # print(f"timesteps: {timesteps.shape, t.dtype, t.unsqueeze(0).shape}")
#                 control = self.control_net(x=current_latent.to(torch.float32), hint=current_hint.to(torch.float32),
#                                            timesteps=t.unsqueeze(0), context=cond_txt.to(torch.float32))
#                 control = [c * scale for c, scale in zip(control, self.control_scales)]
#                 for c in control:print(c.shape) 
#                 all_controls[t.item()][i] = control
...
#                     # control_tensors = [all_controls[i] for i in idx]
#                     # control_tensors = all_controls[key]
#                     control_tensors = [control_t[j] for j in idx]
#                     _noise_pred = self.unet(
#                         latent_model_input[:, idx],
#                         t,
#                         encoder_hidden_states=image_embeddings,

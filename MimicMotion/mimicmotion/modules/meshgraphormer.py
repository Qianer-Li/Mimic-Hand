...
args = Namespace(
    num_workers=1,
    img_scale_factor=1,
    image_file_or_path=os.path.join(mimicmotion_root, 'MeshGraphormer', 'samples', 'hand'), 
    model_name_or_path=os.path.join(mimicmotion_root, 'MeshGraphormer', 'src', 'modeling', 'bert', 'bert-base-uncased'),
    resume_checkpoint=os.path.join(mimicmotion_root, 'MeshGraphormer', 'models', 'graphormer_release', 'graphormer_hand_state_dict.bin'),
    output_dir='output/',
    config_name='',
    a='hrnet-w64',
    arch='hrnet-w64',
...
class MeshGraphormerMediapipe(Preprocessor):
    ...
    def get_depth(self, input_dir, file_name, padding):
        ...
        print(f"hand_landmarks_list, handedness_list shape: {hand_landmarks_list.shape, handedness_list.shape}")
    ...
    def process_hand_landmarks(self, hand_landmarks_list, handedness_list):
        if hand_landmarks_list is None or handedness_list is None:
            return None, None
        
        # 将手部关键点和得分进行排序，依据是手部得分的第10个关键点的得分
        # 注意：这里假设 hands_score 的形状为 (N, M) 其中 M 是关键点数量
        # 例如, N 为检测到的手部的数量，M 为每只手部的关键点数目（如21）

        # 计算每只手的得分的平均值或最大值，选择排序依据
        hand_scores = [score[9] for score in hand_landmarks_list]  # 使用第10个关键点的得分作为排序依据

        # 进行排序
        sorted_indices = np.argsort(hand_scores)[::-1]

        # 按得分排序
        hand_landmarks_list = [hand_landmarks_list[i] for i in sorted_indices]
        handedness_list = [handedness_list[i] for i in sorted_indices]

        return hand_landmarks_list, handedness_list

    # TODO：
    def get_hand(self, image, padding, file_name="Image"):
        info = {}
        
        detection_result = self.dwdetector.detect(image)
        
        hands = detection_result.get('hands')
        hands_score = detection_result.get('hands_score')
        # print(f"hand, hand_score: {hands, detection_result.get('left_hands')}")
        
        # raw_image = image.numpy_view()
        # raw_image = resize_image(image, self.image_resolution)
        raw_image = image
        H, W, C = raw_image.shape
        print('H, W', H, W)
        # HANDLANDMARKS CAN BE EMPTY, HANDLE THIS!
        if len(hands) == 0:
            ...
            print("Can!!!!! detect hands for original image {}".format(file_name))
        raw_image = raw_image[:, :, :3]
        
        padded_image = np.zeros((H*2, W*2, 3))
        padded_image[int(1/2 * H):int(3/2 * H), int(1/2 * W):int(3/2 * W)] = raw_image
        
        # print(f"hand_landmarks_list, handedness_list shape: {left_hands_score, left_hands_landmarks_list}")
        ...
        for idx in range(len(hands)):
            hand = hands[idx]
            hand_score = hands_score[idx]
            height, width, _ = raw_image.shape
            # print(f"hand, hand_score: {hand, hand_score}")
            ...
            if crop_len < 224:
                ...
                graphormer_input = cv2.resize(cropped, (224, 224), interpolation=cv2.INTER_AREA)
            scale = crop_len/224
            cropped_depthmap, pred_2d_keypoints = self.run_inference(graphormer_input.astype(np.uint8), self._model, self.mano_model,\
                                                                     self.mesh_sampler, scale, int(crop_len)) 
            if cropped_depthmap is None:
                print("Depth reconstruction failed for image {}".format(file_name))
                depth_failure = True
                break
            ...
            if idx < num_hands / 2:
                cropped_depthmap = cv2.flip(cropped_depthmap, 1)
            resized_cropped_depthmap = cv2.resize(cropped_depthmap, (int(crop_len), int(crop_len)),\
    ...
    def get_hands(self, image, padding, file_name="Image"):
        info = {}

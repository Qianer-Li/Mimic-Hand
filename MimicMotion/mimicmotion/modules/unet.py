...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    Parameters:
        ...
        一个时空 UNet 模型，接收噪声视频帧、条件状态和时间步，返回样本形状输出。
    """
    ...
    def forward(
        ...
        """
        if sample.size(0)==2 and control is not None:
        ...
        if pose_latents is not None and control_latents is not None :
            sample = sample + pose_latents + control_latents
        ...
        image_only_indicator = torch.ones(batch_size, num_frames, dtype=sample.dtype, device=sample.device) \
            if image_only_indicator else torch.zeros(batch_size, num_frames, dtype=sample.dtype,\
        ...
        if control is not None:
            print(len(control), len(control[0]), sample.shape)
            flag = len(control[0])-1
            for idx in range(sample.size(0)):
                # smaple_control = control[idx].pop()
                smaple_control = control[idx][flag].clone()
                # print(smaple_control.sum())
                # print(sample[idx:idx+1, :, :, :].shape, smaple_control.shape)
                sample[idx:idx+1, :, :, :]=sample[idx:idx+1, :, :, :]+smaple_control
            flag-=1
            
        # print("sample", sample.shape)
        # for i, res_sample in enumerate(down_block_res_samples):
        #     print(f'  res_samples[{i}].shape: {res_sample.shape}')
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            # for idx in range(len(res_samples)):
            #     # temp_flag = flag
            #     true_idx = len(res_samples)-idx-1
            #     for iddx in range(res_samples[idx].size(0)):
            #         # smaple_control = control[iddx].pop()
            #         smaple_control = control[iddx][flag].clone()
            #         # temp_flag-=1
            #         for c in control[iddx]:print('control[iddx]', c.shape) 
            #         print("smaple_control", flag+1, true_idx,\
            #               res_samples[true_idx][iddx:iddx+1, :, :, :].shape, smaple_control.shape)
            #         res_samples[true_idx][iddx:iddx+1, :, :, :] = res_samples[true_idx][iddx:iddx+1, :, :, :]+smaple_control
            #     flag-=1
